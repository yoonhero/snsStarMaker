...
parser = argparse.ArgumentParser(description='KoGPT-2')
...
class KoGPT2Chat(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        mask_out = torch.where(
            mask_3d == 1, out, self.neg*torch.ones_like(out))
    ...
    def configure_optimizers(self):
        ...
        optimizer = AdamW(optimizer_grouped_parameters,
                          lr=self.hparams_.lr, correct_bias=True)

        num_train_steps = len(self.train_dataloader()) * 20
        num_warmpu_steps = int(num_train_steps * self.hparams_.warmup_ratio)

        scheduler = get_cosine_schedule_with_warmup(
            optimizer,
            num_warmup_steps=num_warmpu_steps, num_training_steps=num_train_steps
        )
    ...
    def train_dataloader(self):
        data = pd.read_csv("instagram_chat.csv")
        self.train_set = CharDataset(data, max_len=self.hparams_.max_len)
        train_dataloader = DataLoader(
            self.train_set, batch_size=self.hparams_.batch_size, num_workers=1, collate_fn=self._collate_fn
        )
